...
export default function Dashboard({ onNavigate }: { onNavigate: (view: any) => void }) {
  ...
  const stats = [
    { label: "Clients Totaux", value: totalClients, color: "text-gray-800 dark:text-gray-200" },
    { label: "Interventions", value: totalInterventions, color: "text-gray-800 dark:text-gray-200" },
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-50 dark:bg-gray-950 overflow-y-auto">
      ...
      <header className="shrink-0 bg-white dark:bg-gray-900 border-b dark:border-gray-800">
        ...
        {/* Mobile — grille 2×2 */}
        <div className="md:hidden grid grid-cols-2 divide-x divide-y divide-gray-100 dark:divide-gray-800">
          {stats.map((s) => (
      ...
      <main className="p-3 md:p-6">
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden mt-2">
          <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50">
            <h2 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Dernières interventions</h2>
            <button
              onClick={() => onNavigate("interventions")}
              className="text-blue-600 hover:text-blue-700 dark:text-blue-400 text-xs font-bold"
            >
              Voir tout
            </button>
          </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-gray-50 dark:bg-gray-950 text-gray-400 uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {interventions.slice(0, 5).map(int => (
                  <tr key={int.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/80 dark:bg-gray-900/80">
                    ...
                    <td className="px-6 py-3">
                      <span className={`px-2 py-1 rounded text-[10px] font-bold ${
                        int.status === "Livré" || int.status === "Résolu" ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800" :
                        int.status === "En cours" ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-b dark:border-gray-800lue-100" :
            ...
            {interventions.slice(0, 5).map(int => (
              <div key={int.id} className="p-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 mt-0.5">{int.client.prenom} {int.client.nom}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">{int.equipement.marque} {int.equipement.modele}</p>
