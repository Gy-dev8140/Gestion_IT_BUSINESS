...
const Input = ({ label, name, type = "text", required = false, className = "col-span-1" }: any) => (
    <div className={`space-y-1 ${className}`}>
      ...
      <input type={type} name={name} required={required} className="w-full p-2.5 border dark:border-gray-700 rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
    </div>
  );
  ...
  const Textarea = ({ label, name, required = false, className = "col-span-2" }: any) => (
    <div className={`space-y-1 ${className}`}>
      <label className="text-[10px] text-gray-400 font-bold uppercase">{label} {required && "*"}</label>
      <textarea name={name} required={required} rows={3} className="w-full p-2.5 border dark:border-gray-700 rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
    </div>
  );
  ...
  const Select = ({ label, name, options, required = false, className = "col-span-1" }: any) => (
    <div className={`space-y-1 ${className}`}>
      <label className="text-[10px] text-gray-400 font-bold uppercase">{label} {required && "*"}</label>
      <select name={name} required={required} className="w-full p-2.5 border dark:border-gray-700 rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
...
export default function FormSiteWeb({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const details = {
      typeSite: getS('typeSite'),
      budget: getS('budget'),
      delai: getS('delai'),
      fonctionnalites: getS('fonctionnalites'),
      exemples: getS('exemples')
    };
    ...
    try {
      ...
      const demande: DemandeService = {
        ticketId,
        client,
        serviceType: "CONCEPTION DE SITE WEB",
        details,
        status: "Nouveau",
        createdAt: new Date().toISOString()
      };
      ...
      sendWhatsAppConfirmation(client.whatsapp || client.telephone, ticketId, "CONCEPTION DE SITE WEB");
      setSuccess(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden bg-gray-50 dark:bg-gray-950">
      <div className="shrink-0 bg-white dark:bg-gray-900 px-4 py-3 border-b dark:border-gray-800 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">Nouvelle Demande : Site Web</h2>
      </div>

      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
          <section className="space-y-4 md:col-span-1">
            <h3 className="text-xs font-bold text-emerald-600 uppercase border-b border-emerald-100 dark:border-emerald-800 pb-1">Informations Client</h3>
          ...
          <section className="space-y-4 md:col-span-2">
            <h3 className="text-xs font-bold text-emerald-600 uppercase border-b border-emerald-100 dark:border-emerald-800 pb-1">Détails du Projet</h3>
            <div className="grid grid-cols-2 gap-3">
              <Select label="Type de site" name="typeSite" required options={["Vitrine", "E-commerce", "Blog / Portfolio", "Sur mesure", "Refonte"]} />
              <Select label="Budget estimé" name="budget" required options={["< 200k FCFA", "200k - 500k FCFA", "500k - 1M FCFA", "> 1M FCFA"]} />
              <Input label="Délai souhaité" name="delai" required />
              <Textarea label="Fonctionnalités clés (ex: Paiement, Réservation...)" name="fonctionnalites" required className="col-span-2" />
              <Input label="Exemples de sites que vous aimez (liens)" name="exemples" className="col-span-2" />
            </div>
          </section>
        </div>
      </main>
      ...
      <footer className="shrink-0 bg-white dark:bg-gray-900 border-t dark:border-gray-800 px-4 md:px-8 py-3 flex items-center justify-end gap-3 safe-area-bottom">
        <button type="submit" disabled={loading} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full text-sm font-bold shadow-lg shadow-emerald-200 flex items-center justify-center gap-2">
