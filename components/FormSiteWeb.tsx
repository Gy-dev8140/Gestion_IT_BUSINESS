...
const Input = ({ label, name, type = "text", required = false, className = "col-span-1" }: any) => (
    <div className={`space-y-1 ${className}`}>
      ...
      <input type={type} name={name} required={required} className="w-full p-2.5 border rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500" />
    </div>
  );
  ...
  const Textarea = ({ label, name, required = false, className = "col-span-2" }: any) => (
    <div className={`space-y-1 ${className}`}>
      <label className="text-[10px] text-gray-400 font-bold uppercase">{label} {required && "*"}</label>
      <textarea name={name} required={required} rows={3} className="w-full p-2.5 border rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500" />
    </div>
  );
  ...
  const Select = ({ label, name, options, required = false, className = "col-span-1" }: any) => (
    <div className={`space-y-1 ${className}`}>
      <label className="text-[10px] text-gray-400 font-bold uppercase">{label} {required && "*"}</label>
      <select name={name} required={required} className="w-full p-2.5 border rounded-lg text-xs outline-none focus:ring-1 focus:ring-emerald-500">
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden bg-gray-50">
      <div className="shrink-0 bg-white px-4 py-3 border-b flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-gray-900 tracking-tight">Nouvelle Demande : Site Web</h2>
      </div>

      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
          <section className="space-y-4 md:col-span-1">
            <h3 className="text-xs font-bold text-emerald-600 uppercase border-b border-emerald-100 pb-1">Informations Client</h3>
          ...
          <section className="space-y-4 md:col-span-2">
            <h3 className="text-xs font-bold text-emerald-600 uppercase border-b border-emerald-100 pb-1">Détails du Projet</h3>
            <div className="grid grid-cols-2 gap-3">
